...
void ReadFlowInput(){
	if (flow_input.idx < flow_num){
		//cout << "src " <<flow_input.src << "dst " << flow_input.dst << endl;
...
int main(int argc, char *argv[])
{
...
#endif
	{
		...
		while (!conf.eof())
		{
			...
			}else if (key.compare("P4THMETER_THR") == 0){
				conf >> prioMeter_thr;
				std::cout << "P4THMETER_THR\t\t\t\t" << prioMeter_thr << '\n';
			}
			fflush(stdout);
		}
		conf.close();
	...
	else if (cc_mode == 13) // prioMeter
		IntHeader::mode = IntHeader::P4THMETER;
	...
	for (uint32_t i = 0; i < node_num; i++){
		if (n.Get(i)->GetNodeType() == 0){ // is server
			...
			rdmaHw->SetPrioMeterThresh(prioMeter_thr);
	...
	for (uint32_t i = 0; i < node_num; i++){
		if (n.Get(i)->GetNodeType() == 1){ // switch
			...
			sw->SetAttribute("prioMeterThreshold", UintegerValue(prioMeter_thr));
		}
	}
	...
	flow_input.idx = 0;
	//cout << " #flows ..." << flow_num <<endl;
	if (flow_num > 0){
		//cout << "start reading flows ..." << endl;
		ReadFlowInput();
		Simulator::Schedule(Seconds(flow_input.start_time)-Simulator::Now(), ScheduleFlowInputs);
	}
