...
TypeId RdmaHw::GetTypeId (void)
{
	static TypeId tid = TypeId ("ns3::RdmaHw")
		...
		.AddAttribute("PintSmplThresh",
				...
				MakeUintegerChecker<uint32_t>())
		.AddAttribute("PrioMeterTimerThresh",
				"PrioMeter timer threshold",
				UintegerValue(1000),
				MakeUintegerAccessor(&RdmaHw::prioMeter_thr),
				MakeUintegerChecker<uint32_t>())
		;
	return tid;
}
		...
		if (ih.nhop <= IntHeader::maxHop){
			...
			if (!m_multipleRate){
				...
				if (updated_any){
					...
					#endif
					int64_t timerNow =Simulator::Now().GetMicroSeconds ();
				//	if (qp->dip.Get()==184549889 || qp->dip.Get()==184550145)
						//printf(" rate: %.3lf Timer: %lu\n", qp->hp.m_curRate.GetBitRate()*1e-9, timerNow);
					#if PRINT_LOG
					if (print)
			...
			if (updated_any){
					int64_t timerNow =Simulator::Now().GetMicroSeconds ();
       					//printf(" rate: %0.3lf Timer: %lu\n", qp->m_rate.GetBitRate() * 1e-9, timerNow);
 ...
 * P4THMETER
 ********************/
void RdmaHw::SetPrioMeterThresh(uint32_t thr){
       prioMeter_thr = thr;
}
std::string intToIpAddress(uint32_t ipAddress) {
    uint8_t octets[4];

    // Extract octets from the integer
    octets[0] = (ipAddress >> 24) & 0xFF;
    octets[1] = (ipAddress >> 16) & 0xFF;
    octets[2] = (ipAddress >> 8) & 0xFF;
    octets[3] = ipAddress & 0xFF;

    // Format the octets as a dotted-decimal IP address
    std::stringstream ss;
...
void RdmaHw::ReportRate(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch){
	uint32_t ack_seq = ch.ack.seq;
		...
		uint8_t QL=124;
       		FlowIdTag t;
       		p->PeekPacketTag(t);
		// uint32_t PrioMeter_thr=1000000;
       	//printf("SIP:%d , dip: %d, PG:%d ,  PrioMeterRate: %0.4lf\n",qp->sip.Get(), qp->dip.Get(), qIndex , ih.PrioMeterGetRate()*8.0*10000.0*1e-9);
		int64_t timerNow =Simulator::Now().GetMicroSeconds ();
		uint64_t tmr = Simulator::Now().GetTimeStep();
		

		/* to show the rates, we do as follows;
		1. extract the rate from packet header and use offset to find the rate of each PG
		2. perform left shift according to QL
		3. show rate of each PG
		*/
		uint8_t qCnt=8;
		uint64_t tempQuantizedRates;
		tempQuantizedRates = ih.PrioMeterGetRate();
		//printf(" Measured Rate: %016llx \n",tempQuantizedRates);
		//printf(" prioMeter_thr: %d \n",prioMeter_thr);
		uint64_t PGrate[qCnt]; // Array to store individual bytes
		double sumRate=0;
		if(tempQuantizedRates>0){
			uint64_t dt = tmr - preTime;
			if ( dt > prioMeter_thr){
				preTime=tmr;
				for (int i = 0; i < qCnt; ++i) {
					PGrate[i] = (tempQuantizedRates >> (i * 8)) & 0xFF;
					//printf(" PGrate[%d]: %lx \t",i,PGrate[i]);
					PGrate[i]<<= QL;
					//printf(" PG:%d , rate: %0.4lf Gbps Timer: %lu\n", i , PGrate[i]*8.0/prioMeter_thr,timerNow);
					//printf(" PG:%d , rate: %0.4lf Gbps \n", i , PGrate[i]*8.0/prioMeter_thr);
					PGrate[i]= PGrate[i]*8.0/prioMeter_thr;
					sumRate+=PGrate[i];
				}

				//printf(" Time: %lu SumRate on endhost from host %s to %s : %lf Gbps \n",timerNow,intToIpAddress(qp->sip.Get()).c_str(),intToIpAddress(qp->dip.Get()).c_str(),sumRate);
		              // printf(" SumRate on endhost from host %s to %s : %lf Gbps \n",intToIpAddress(qp->sip.Get()).c_str(),intToIpAddress(qp->dip.Get()).c_str(),sumRate);
			}
			
	//	printf("%i.%i.%i.%i\n", (qp->sip.Get() >> 24) & 0xFF,  (qp->sip.Get() >> 16) & 0xFF, (qp->sip.Get() >> 8) & 0xFF, qp->sip.Get() & 0xFF);
	//	printf("IP %s\n",intToIpAddress(qp->sip.Get()).c_str());

		}
        
		

       		//printf(" PG_%d rate: %0.3lf\n",qIndex, ih.PrioMeterGetRate()*8.0*1e-3);
		// if (qp->dip.Get()==184549889 || qp->dip.Get()==184550145)
      	// 	printf(" PG:%d , rate: %0.4lf Timer: %lu\n", qIndex , ih.PrioMeterGetRate()*8.0/5000,timerNow);
	
		/*if(qp->sip.Get()== 184549377 && qp->dip.Get()==184549889)
		{
       			printf("path 0 -> 2, PG:%d ,  PrioMeterRate: %0.4lf  time: %u\n", qIndex , ih.PrioMeterGetRate()*8.0*thr*1e-9,timerNow);
       			//printf("path 0 -> 2, PG:%d ,  PrioMeterRate: %d  time: %u\n", qIndex , ih.PrioMeterGetRate(),timerNow);
		}
		else if(qp->sip.Get()== 184549377 && qp->dip.Get()==184550145)
		{
       			//printf("path 0 -> 3, PG:%d ,  PrioMeterRate: %0.4lf  time: %u\n", qIndex , ih.PrioMeterGetRate()*8.0*thr*1e-9,timerNow);
       			printf("path 0 -> 3, PG:%d ,  PrioMeterRate: %d  time: %u\n", qIndex , ih.PrioMeterGetRate(),timerNow);
		}
		else if(qp->sip.Get()== 184549633 && qp->dip.Get()==184549889)
		{
       			//printf("path 1 -> 2, PG:%d ,  PrioMeterRate: %0.4lf  time: %u\n", qIndex , ih.PrioMeterGetRate()*8.0*thr*1e-9,timerNow);
       			printf("path 1 -> 2, PG:%d ,  PrioMeterRate: %d  time: %u\n", qIndex , ih.PrioMeterGetRate(),timerNow);
		}
		else if(qp->sip.Get()== 184549633 && qp->dip.Get()==184550145)
		{
       			printf("path 1 -> 3, PG:%d ,  PrioMeterRate: %d  time: %u\n", qIndex , ih.PrioMeterGetRate(),timerNow);
       			//printf("path 1 -> 3, PG:%d ,  PrioMeterRate: %0.4lf  time: %u\n", qIndex , ih.PrioMeterGetRate()*8.0*thr*1e-9,timerNow);
		}
*/
	}
