...
int IntHeader::prioMeter_bytes = 8;
...
uint32_t IntHeader::GetStaticSize(){
	if (mode == NORMAL){
		...
		return sizeof(pint);
	}else if (mode == P4THMETER){
		return sizeof(prioMeter);
	}else {
		return 0;
	}
}
...
// set/get methods for PrioMeter
uint64_t IntHeader::PrioMeterGetRate(){
	if (mode == P4THMETER)
		return prioMeter.rate;
	return 0;
}
void IntHeader::PrioMeterSetRate(uint64_t rate){
	if (mode == P4THMETER)
		prioMeter.rate = rate;
}
}
